...
func shoot( Apoint ):
	...
	##bullet.set_position(bullet.position + NewPoint * Bvector.length() + Coll_Offset)  
	...
	bullet.set_position(Parent.position +  NewPoint * Bvector.length() + Coll_Offset)  # do after with parent.position to stop flash at origin